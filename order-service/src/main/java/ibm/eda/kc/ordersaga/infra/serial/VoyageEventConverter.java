...
public class VoyageEventConverter implements MessageConverter {
    ...
    public boolean canConvert(Message<?> in, Type target) {
        return in.getMetadata(IncomingKafkaRecordMetadata.class).isPresent()
                && target.equals(VoyageReservationEvent.class);
    }
    ...
    public Message<?> convert(Message<?> in, Type target) {
        ...
        return in.withPayload(new VoyageReservationEvent(sagaId, messageId, ((VoyageReservationEvent) in.getPayload()).status, metadata.getHeaders()));
    }
