...
public class ContainerEventConverter implements MessageConverter {
    ...
    public boolean canConvert(Message<?> in, Type target) {
        return in.getMetadata(IncomingKafkaRecordMetadata.class).isPresent()
                && target.equals(ContainerReservationEvent.class);
    }
    ...
    public Message<?> convert(Message<?> in, Type target) {
        ...
        return in.withPayload(new ContainerReservationEvent(sagaId, messageId, ((ContainerReservationEvent) in.getPayload()).status, metadata.getHeaders()));
    }
